...
def admin_dashboard():
    STATUS_COL = {'bidding':'#fbbf24','playing':'#4ade80','lobby':'#60a5fa'}
    TRUMP_SYM  = {'S':'&#9824;','D':'&#9830;','C':'&#9827;','H':'&#9829;'}
    rows = ''
    for code, g in games.items():
        ...
        rnd       = str(g.ridx+1)+'/'+str(len(g.rseq)) if g.rseq else '-'
        trump_sym = TRUMP_SYM.get(g.trump,'')
        col       = STATUS_COL.get(g.status,'#f87171')
        sk        = str(skipped) if skipped else '-'
        rows += ('<tr>'
            + '<td><b>'+code+'</b></td>'
            + '<td>'+str(total)+'</td>'
            + '<td style="color:#4ade80">'+str(connected)+'</td>'
            + '<td>'+sk+'</td>'
            + '<td>'+rnd+'</td>'
            + '<td>'+trump_sym+' '+(g.trump or '-')+'</td>'
            + '<td><span style="color:'+col+'">'+g.status+'</span></td>'
            + '</tr>')
    ...
    table = ('<table><thead><tr><th>ROOM</th><th>PLAYERS</th><th>ONLINE</th>'
             '<th>SKIPPED</th><th>ROUND</th><th>TRUMP</th><th>STATUS</th></tr></thead>'
             '<tbody>'+rows+'</tbody></table>') if games else '<div class="empty">No active rooms</div>'
    css = ('body{background:#0d2818;color:#f0e6c8;font-family:monospace;padding:16px;}'
           ...
           '.stat{background:rgba(0,0,0,0.4);border:1px solid rgba(201,168,76,0.3);border-radius:8px;padding:12px 16px;min-width:120px;}'
           ...
           'td{padding:8px 10px;border-bottom:1px solid rgba(201,168,76,0.08);}'
           'tr:hover td{background:rgba(201,168,76,0.05);}'
           '.empty{text-align:center;padding:40px;color:rgba(240,230,200,0.3);}'
           '.note{font-size:12px;color:rgba(240,230,200,0.4);margin-top:12px;}')
    html = ('<!DOCTYPE html><html><head>'
            ...
            '<div class="sub">Live — auto refreshes every 5 seconds</div>'
            '<div class="stats">'
            '<div class="stat"><div class="sv">'+str(len(games))+'</div><div class="sl">Active Rooms</div></div>'
            '<div class="stat"><div class="sv">'+str(total_players)+'</div><div class="sl">Total Players</div></div>'
            '<div class="stat"><div class="sv" style="color:#4ade80">'+str(active_players)+'</div><div class="sl">Connected Now</div></div>'
            '</div>'
            + table +
            '<div class="note">Storage: '+redis_status+'</div>'
            '</body></html>')
    return html
