...
def save_games():
    try:
        ...
        for code, g in games.items():
            data[code] = {
                'code': g.code, 'host_sid': g.host_sid, 'status': g.status,
                'skipped': list(g.skipped), 'waiting': list(g.waiting),
                ...
                'players': [{
                    'sid': p['sid'], 'name': p['name'], 'is_host': p['is_host'],
                    'score': p['score'], 'bid': p['bid'], 'won': p['won'],
                    'cards': p['cards'], 'connected': False
                } for p in g.players]
            }
        payload = json.dumps(data)
        if _redis:
            _redis.set(REDIS_KEY, payload, ex=86400)  # expire after 24h
        else:
...
def load_games():
    try:
        ...
        for code, d in data.items():
            if d['status'] in ('lobby', 'game_over'):
                continue
            g = Game.__new__(Game)
            ...
            g.skipped = set(d.get('skipped', []))
            g.waiting = set(d.get('waiting', []))
            restored[code] = g
...
class Game:
    ...
    def add_player(self, sid, name, host=False):
        self.players.append({
            'sid': sid, 'name': name, 'is_host': host,
            'score': 0, 'bid': None, 'won': 0,
    ...
    def I(self, sid):
        return next((i for i, p in enumerate(self.players) if p['sid'] == sid), -1)
    ...
    def _deal(self, deck=None):
        ...
        self.trump = TRUMP_CYC[self.ridx % 4]
        self.trick = []
        ...
        for i, p in enumerate(active):
            p['cards'] = deck[i*nc:(i+1)*nc]
        self.tlead = self._next_active((self.didx + 1) % self.N())
        self.cidx = self.tlead
        self.status = 'bidding'

    def advance(self):
        self.ridx += 1
        self.didx = (self.didx + 1) % self.N()
        self._deal()

    def bid(self, sid, amt):
        p = self.P(sid)
        pi = self.I(sid)
        if not p or p['sid'] in self.skipped:
            return False, "Player is skipped"
        if pi != self.cidx or p['bid'] is not None:
        ...
        if pi == self.didx:
            ...
            if others + amt == nc:
                return False, f"Hook rule: cannot bid {amt}"
        p['bid'] = amt
        ...
        if active and all(x['bid'] is not None for x in active):
            self.status = 'playing'
            self.cidx = self._next_active(self.tlead)
        return True, "ok"
    ...
    def resolve(self):
        ...
        for e in self.trick[1:]:
            if beats(e['card'], w['card'], led, self.trump):
                w = e
        wp = self.P(w['sid'])
        wp['won'] += 1
        wi = self.I(w['sid'])
        ...
        # 🎉 Celebration: emit to winner (and room) for trick win
        socketio.emit('celebration', {
            'type': 'trick',
            'winner': w['name'],
            'message': f'{w["name"]} wins the trick!'
        }, room=self.code)

        return w['sid'], w['name'], t

    def done(self):
        return all(len(p['cards']) == 0 for p in self.players if p['sid'] not in self.skipped)

    def last(self):
        return self.ridx >= len(self.rseq) - 1

    def _next_active(self, idx):
        for _ in range(self.N()):
            if self.players[idx]['sid'] not in self.skipped:
                return idx
            idx = (idx + 1) % self.N()
        return idx
    ...
    def score(self):
        ...
        for p in self.players:
            ...
            p['score'] += pts
            res.append({
                'name': p['name'], 'bid': p['bid'], 'won': p['won'],
                'pts': pts, 'total': p['score']
            })
        self.history.append({
            'round': self.ridx + 1, 'cards': nc,
            'trump': self.trump, 'scores': res
        })
        self.status = 'round_end'
        return res

    def final(self):
        return sorted([{'name': p['name'], 'score': p['score']} for p in self.players],
                      key=lambda x: -x['score'])

    def forbidden(self):
        if self.status != 'bidding' or self.cidx != self.didx:
            return None
        nc = self.rseq[self.ridx]
        others = sum(x['bid'] for x in self.players if x['bid'] is not None)
        fb = nc - others
    ...
    def snap(self, fsid=None):
        ...
        for p in self.players:
            ...
            if fsid and p['sid'] == fsid:
                pd['cards'] = p['cards']
            ps.append(pd)
        return {
            'code': self.code, 'status': self.status, 'num_decks': self.num_decks,
            ...
            'didx': self.didx,
            'dealer_name': self.players[self.didx]['name'] if self.status != 'lobby' and self.players else '',
            'host_sid': self.host_sid, 'trick': self.trick, 'players': ps,
            'history': self.history, 'forbidden': self.forbidden(),
            'maxc': self.maxc, 'total_rounds': len(self.rseq)
        }
...
@socketio.on('join')
def on_join(d):
    name = d.get('name', '').strip()[:20]
    code = d.get('code', '').upper().strip()
    if code not in games:
        emit('err', {'msg': f'Room "{code}" not found'})
        return
    g = games[code]
    if g.status != 'lobby':
        emit('err', {'msg': 'Game already started'})
        return
    if g.N() >= 15:
        emit('err', {'msg': 'Room full (15 max)'})
        return
    if any(p['name'].lower() == name.lower() for p in g.players):
        emit('err', {'msg': f'Name "{name}" taken'})
        return
    g.add_player(request.sid, name)
    join_room(code)
    join_room(request.sid)
    emit('joined', {'code': code, 'you': name})
    for p in g.players:
...
def on_start(d):
    code = d.get('code')
    if code not in games:
        emit('err', {'msg': 'Room not found'})
        return
    g = games[code]
    if request.sid != g.host_sid:
        emit('err', {'msg': 'Only host can start'})
        return
    if g.N() < 2:
        emit('err', {'msg': 'Need at least 2 players'})
        return
    g.start()
    bcast(g)
    socketio.emit('toast', {
        'msg': f'Game on! Round 1 — {g.rseq[0]} card. Trump: {SUIT_SYM[g.trump]} {SUIT_NAME[g.trump]}',
        't': 'info'
    }, room=g.code)
...
def on_bid(d):
    ...
    if not isinstance(amt, int):
        emit('err', {'msg': 'Invalid bid'})
        return
    # Fix stale SID on reconnect
    ...
    if g.status == 'playing':
        socketio.emit('toast', {
            'msg': f'All bids in! {g.players[g.tlead]["name"]} leads first.',
            't': 'success'
        }, room=g.code)
    bcast(g)
...
def on_play(d):
    ...
    if msg == 'trick_done':
        ...
        # Small delay for trick result animation
        if os.environ.get('RENDER'):
        ...
        if g.done():
            ...
            if g.last():
                ...
                socketio.emit('celebration', {
                    'type': 'game_over',
                    'winner': res[0]['name'] if res else '',
                    'message': f'Game Over! {res[0]["name"]} wins!'
                }, room=g.code)
                socketio.emit('game_over', {'final': g.final()}, room=g.code)
            else:
                socketio.emit('round_end', {'result': res, 'round': g.ridx + 1}, room=g.code)
        else:
            bcast(g)
    else:
        bcast(g)
...
def on_next(d):
    ...
    nc = g.rseq[g.ridx]
    socketio.emit('toast', {
        'msg': f'Round {g.ridx+1} — {nc} card(s). Trump: {SUIT_SYM[g.trump]} {SUIT_NAME[g.trump]}',
        't': 'info'
    }, room=g.code)
...
def on_close_room(d):
    ...
    g = games[code]
    if request.sid != g.host_sid:
        emit('err', {'msg': 'Only host can close room'})
        return
...
def on_skip(d):
    ...
    g = games[code]
    if request.sid != g.host_sid:
        emit('err', {'msg': 'Only host can skip'})
        return
    p = next((x for x in g.players if x['name'].lower() == name.lower()), None)
    if not p:
        emit('err', {'msg': 'Player not found'})
...
def on_chat(d):
    code = d.get('code')
    msg = d.get('msg', '').strip()[:200]
    if not code or code not in games or not msg:
        return
    g = games[code]
    p = g.P(request.sid)
    if not p:
...
@app.route('/join/<code>')
def joinlink(code):
    return render_template('index.html', join_code=code)
